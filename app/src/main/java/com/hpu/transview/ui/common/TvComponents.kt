 ...
 *
 * ## 用法约束
 * - [isFocused] 必须读**真实焦点状态**：目标节点自己的 `isFocused`（`onFocusChanged` 里
 *   **true / false 都写**的状态），或由这些状态推导出来的值。
 *   **不要用 `hasFocus` 直接当判据** —— 理由同上，Back 会把它清空。
 * - ⚠️ **不要用「只在 isFocused 时写」的单值记录当判据，更不要「先清空记录再要求它被写回」**：
 *   目标**本来就已经聚焦**时 `requestFocus()` 不产生任何焦点变化事件，记录永远等不到写入 ⇒
 *   校验恒判失败 ⇒ 调用方走兜底路径，把焦点推到一个用户并不想去的地方。
 *   真机实测（2026-09-18）：焦点在「上传」标签上按返回，焦点被兜底推进上传内容区，
 *   再按返回又回到标签，如此往复、**永远退不出应用**。
 *   正确做法是把「该节点此刻是否聚焦」如实记下来（无条件写），已聚焦 ⇒ 第一次就判成功。
