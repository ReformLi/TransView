...
### 3.19 App 运行日志本地化（App 调试日志，v1.17；v1.31 接入全链路埋点）
...
v1.17 上线时只把**原有的 8 处 `Log.*`** 换成 `AppLogger.*`（仅 4 个文件），实际覆盖面约 7%；
v1.31 补齐为**全链路关键路径埋点（约 90 个调用点 / 20 个文件）**，详见 §3.19.7。
...
* 设置 → 存储与数据 → **App 调试日志**（`focusKey = "存储与数据:3"`，槽位固定为 `:3`；
  ...
  **总闸**，值持久化在 `SettingsStore.appLogEnabled`（键 `app_log_enabled`），**默认关**。
* 设置 → 存储与数据 → **详细日志**（`focusKey = "存储与数据:9"`，**默认关**，v1.31 新增）。
  **二级开关**，值持久化在 `SettingsStore.detailedLogEnabled`（键 `detailed_log_enabled`）。
  与总闸**正交**：只在总闸开启时有意义；关掉总闸时本标志保持不动，下次开总闸仍按原偏好生效。
  切换时调用 `AppLogger.setDetailed(on)` —— 只翻转一个 `@Volatile` 标志、**不重启写盘引擎**，立即生效。
  * `focusKey` 取 `:9` 而非 `:4` —— 本页约定**新增行一律分配未占用的新编号，绝不重排既有 ID**；
    渲染位置落在 `:3` 之后、编号不连续是安全的（`focusKey` 只用于焦点锚定 / 复位，不参与几何导航）。
* 两个开关都沿用本项目布尔设置项的既有形态：**两项选择弹框（开启 / 关闭）**，而不是 Android 的 `Switch`
  —— TV 端遥控器上「聚焦 + 确定选值」比「左右拨动 Switch」更稳（也避免引入新的焦点节点）。
* 行内副标题（`SettingRow` 新增可选参数 `subtitle`）：
  - App 调试日志：「记录调试日志（仅供排查问题，长期开启可能影响性能）」
  - 详细日志：「额外记录操作轨迹等细节；日志量较大，建议仅在排查时开启」
...
* **立即生效**：切换时同步调用 `AppLogger.setEnabled(on)`，无需重启。
  ...
  并按 `AppLogger.setDetailed(...)` 落地详细模式；启动首行日志会注明「日志=开/关（详细）」。
...
* 引擎启动时执行 `cleanupOldDays()`，**按顺序过两道独立的闸**：
  1. **天数闸**：**只保留最近 7 天**的日期目录，更早的物理删除。由于 `yyyy-MM-dd` 是定长字典序
     == 时间序，按目录名倒排 `drop(7)` 即可，**不需要解析日期**。
  2. **总量闸（v1.31 新增）**：`app_log/` 总量上限 **`MAX_TOTAL_BYTES = 50MB`**。
     v1.31 接入全链路埋点后日志量上升一个数量级，只按天数兜底可能出现「单日几百 MB、7 天累计数 GB」，
     而日志落在**用户可见的沙盒**里（`TransView/Downloads/app_log/`），失控会真的占用户空间。
     超配额时从**最旧**的日期目录开始删 —— `while (total > MAX_TOTAL_BYTES && kept.size > MIN_KEPT_DAYS)`；
     `MIN_KEPT_DAYS = 1`，即**至少保留一天**：超配额宁可略微超出，也不能把日志清到一条不剩
     （否则用户进「其他」页发现目录是空的，等于白开日志）。`app_log/` 根下的散文件（正常不该有）
     也计入总量，避免它们把配额悄悄吃掉。
* **双重保险确保不误删**（两道闸共用 `deleteDayDir()`）：
...
#### 3.19.6 调用点覆盖与两处刻意的例外

**阶段一（v1.17）**：`android.util.Log.*` → `AppLogger.*` 共 **8 处 / 4 个文件**，只是换名、未新增信息：
...
**阶段二（v1.31）**：在上表基础上补齐**全链路关键路径埋点**，总调用点约 **90 处 / 20 个文件**（见 §3.19.7）。
...
> 但**不会死锁也不会无限递归** —— 因为 `AppLogger.v/d/i/w/e` 在调用线程**只 `trySend`（无锁、不等待消费者）**，
> 写失败触发的 `FileLocations.refresh()` 又有 30 秒限流，且只发生在消费侧。

#### 3.19.7 三档等级与全链路埋点（v1.31）

**为什么分级**：v1.17 只有一条总闸，埋点一多就两头不讨好 —— 想记细一点（UI 轨迹、逐次存储探测）
会因高频把 `Channel` 打满、**静默挤掉真正有用的日志**；想保守一点又会在排障时缺关键现场。
v1.31 引入**两级开关 × 三档等级**，把「稳定有价值」与「高频诊断」分开。

| 级别 | 常规模式 | 典型内容 | 频率约束 |
|:--|:--|:--|:--|
| `E` | ✅ 落盘 | 一切失败且需要堆栈现场（上传落盘失败、服务器启动失败、播放器错误码） | 每次事件一条 |
| `W` | ✅ 落盘 | 可自愈 / 可降级的失败（删除失败、目录读失败、时长提取失败、二维码生成失败、外部打开失败） | 每次事件一条 |
| `I` | ✅ 落盘 | **状态跃迁**（服务器启停 / 端口切换、上传开始与结果、对账统计、存储降级与恢复）与**破坏性操作留痕**（删除成功、清空 / 清理确认） | 每次事件一条 |
| `D` | ✅ 落盘 | 较细的诊断（卷扫描结果、手动对账触发、切图、上传残留清理条数） | 每次操作一条 |
| `V` | ❌ 仅详细模式 | **UI 操作轨迹**（切标签、进出文件夹、打开视频 / 图片、设置项变更）、**播放器全链路**、逐次存储探测、逐文件索引 | **可高频**，故默认关闭 |

**硬约束**：高频路径一律用 `V`，且**禁止记「每字节 / 每毫秒」级事件** —— 例如套接字输入流的
`read()`、上传进度轮询（600ms 一次）都**不打点**。理由：`write()` 走的是有界 `Channel(4096)`，
队列满即**静默丢弃**，高频打点不会阻塞业务，但会把队列打满并挤掉真正有用的日志，而丢弃是**不可观测**的。

**覆盖点分布（约 90 处 / 20 文件）**：

| 模块 | 文件（处数） | 重点 |
|:--|:--|:--|
| 服务器 | `TransHttpServer`(14) / `ServerController`(13) / `ZipExtractor`(3) / `UploadStorage`(2) | 鉴权失败、上传开始 / 中断 / 落盘、ZIP 解压结果、启停与端口切换 |
| 数据层 | `SyncManager`(6) / `MediaRepository`(2) | 对账统计、僵尸记录、外部删除样例、孤立索引清理 |
| 存储 | `FileUtils`(10) / `FileStorage`(1) | 卷发现、写探针、降级 / 恢复、跨卷移动失败 |
| UI | `LibraryScreen`(8) / `MainScreen`(2) / `UploadScreen`(2) / `SettingsScreen`(2) / `ImageViewerActivity`(2) | 删除结果、进出目录、打开媒体、设置变更、标签切换 |
| 播放器 / 启动 | `PlayerActivity`(5) / `MainActivity`(4) / `TransViewApp`(4) / `ServerService`(5) / `BootReceiver`(2) | 播放错误与连播、权限与通知、启动对账、开机自启、USB 插拔 |

**取舍说明**：服务器模块选择在**聚合点**记录而非逐分支铺开 —— 例如 `ZipExtractor` 的 8 个失败分支
统一压成 `Outcome.kind + message`，由 `TransHttpServer` 记**一条**并带上 `outcome.message`
（避免 8 个散点日志 + 重复代码）。同理，删除 / 清空 / 清理这类**破坏性操作**只在结果处记 `I`，
不在「打开确认框」处记 —— 用户点了取消不算执行。逐次 `existsForPath` 探测（每次对账数千次）
**不记**：它属于「每文件」级，违反上面的频率硬约束。
